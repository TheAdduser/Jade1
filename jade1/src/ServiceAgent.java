...
class EsperantoCyclicBehaviour extends CyclicBehaviour
{
	ServiceAgent agent;
	public EsperantoCyclicBehaviour(ServiceAgent agent)
	{
		this.agent = agent;
	}
	public void action()
	{
		...
		else
		{
			...
			try
			{
				response = agent.makeRequest("fd-epo-eng",content);
			}
			catch (NumberFormatException ex)
			{
				response = ex.getMessage();
